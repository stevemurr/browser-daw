...
$(BUILD_DIR)/test-plugins: engine/test_plugins.c $(ENGINE_SRCS) $(UNITY_SRC) | $(BUILD_DIR)
	$(CC) $(CFLAGS) $^ -lm -o $@

# ── Test targets ─────────────────────────────────────────────────────────────

test-c: $(BUILD_DIR)/test-engine $(BUILD_DIR)/test-seek $(BUILD_DIR)/test-plugins
	@echo "--- engine tests ---"
	...
	@echo "--- plugin tests ---"
	@LLVM_PROFILE_FILE=$(BUILD_DIR)/plugins.profraw $(BUILD_DIR)/test-plugins
	@$(LLVM_PROFDATA) merge -sparse \
	    $(BUILD_DIR)/engine.profraw \
	    $(BUILD_DIR)/seek.profraw \
	    $(BUILD_DIR)/plugins.profraw \
